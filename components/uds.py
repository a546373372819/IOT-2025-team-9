...
def uds_callback(name, distance):
    t = time.localtime()
    print("="*20)
    print(f"Timestamp: {time.strftime('%H:%M:%S', t)}")
    print(f"{name}: Distance: {distance} cm")

def run_uds(name, settings, threads, stop_event):
    if settings['simulated']:
        print("Starting UDS simulator")
        uds_thread = threading.Thread(target=run_uds_simulator, args=(lambda distance: uds_callback(name, distance), stop_event))
        uds_thread.start()
        ...
        uds_thread = threading.Thread(target=run_uds_loop, args=(uds, lambda distance: uds_callback(name, distance), stop_event))
        uds_thread.start()
        threads.append(uds_thread)
